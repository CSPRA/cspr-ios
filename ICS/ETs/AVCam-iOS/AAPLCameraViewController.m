	...
	dispatch_async( self.sessionQueue, ^{
		...
		[self.stillImageOutput captureStillImageAsynchronouslyFromConnection:connection completionHandler:^( CMSampleBufferRef imageDataSampleBuffer, NSError *error ) {
			if ( imageDataSampleBuffer ) {
                ...
                ScanIDProofViewController * scanIDVC = [self.storyboard instantiateViewControllerWithIdentifier:kScanIDProofVCIdentifier];
                UIImage *finalImage = [UIImage imageWithData:imageData];
                scanIDVC.imageToOCR = finalImage;
                [self.navigationController pushViewController:scanIDVC animated:YES];
                return ;
				[PHPhotoLibrary requestAuthorization:^( PHAuthorizationStatus status ) {
					if ( status == PHAuthorizationStatusAuthorized ) {
						...
						if ( [PHAssetCreationRequest class] ) {
							...
							NSString *temporaryFileName = icsOCRTempImageKey;
